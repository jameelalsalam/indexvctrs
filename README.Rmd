...
Under the hood, these objects are actually data frames with additional attributes indicating which columns are the indices and a `value` column which is the value. Unlike 'normal' vector math, which does matching and recycling by position, the DSL operations match and recycle along corresponding indices, akin to joins.
